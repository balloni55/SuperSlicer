...
std::string CoolingBuffer::process_layer(const std::string &gcode, size_t layer_id, bool append_time_only)
{
    std::vector<PerExtruderAdjustments> per_extruder_adjustments = this->parse_layer_gcode(gcode, m_current_pos);
    if (append_time_only) {
        saved_layer_time.clear();
        for (PerExtruderAdjustments& adj : per_extruder_adjustments) {
            saved_layer_time[adj.extruder_id] = adj.elapsed_time_total();
        }
        return gcode;
    } else {
        if (!saved_layer_time.empty()) {
            for (PerExtruderAdjustments& adj : per_extruder_adjustments) {
                auto it = saved_layer_time.find(adj.extruder_id);
                if (it != saved_layer_time.end()) {
